 ...
 *
 * Standard (std):  sort by CSV overall_rank ascending (pre-calculated board order)
 * Half PPR / PPR:  sort by draft_value_score_{format} descending (higher = better)
...
function csvRankOf(player, fallback = 9999) {
  const raw =
    player?.csvOverallRank ?? player?.overall_rank ?? player?.rank ?? fallback
 ...
 *
 * std:      preserve CSV overall_rank ascending (Allen stays ~12, not PPG order)
 *           unless forceValueSort (weekly board) — then sort like half/ppr
 * half/ppr: sort by draft_value_score_{format} descending, reassign ranks 1..N
 * Positional: always by projected_ppg (format-invariant)
 *
 * @param {Array<object>} players
 * @param {'std'|'half_ppr'|'ppr'|string} format
 * @param {{ superflex?: boolean, forceValueSort?: boolean }} [options]
 */
export function rankPlayersByFormat(players, format, options = {}) {
  const { superflex = false, forceValueSort = false } = options
  ...
  if (fmt === 'std' && !superflex && !forceValueSort) {
    // Trust pre-calculated CSV overall_rank (1 = best). Do NOT re-sort by PPG
    // or draft_value_score_std — those inflate QBs like Allen to the top.
    list.sort((a, b) => {
      const ra = csvRankOf(a)
      const rb = csvRankOf(b)
    ...
    list.forEach((p, i) => {
      ...
      const csvRank = csvRankOf(p, i + 1)
      p.overall_rank = csvRank
      p.rank = csvRank
    ...
    // Half PPR / PPR / Superflex / weekly forceValueSort: higher draft_value = better
    list.sort((a, b) => {
  ...
  // Positional rank by projected_ppg (same in every format)
  const byPos = {}
  for (const p of list) {
