 ...
 * Format PPG / draft values are filled later via mergeWeeklyWithLive().
 * Sort order comes from Rank until a scoring format re-rank runs.
 */
...
/** Attach ROS per-format PPG / draft value / injury onto weekly rows. */
function mergeWeeklyWithLive(weeklyPlayers, livePlayers) {
  ...
  return (weeklyPlayers || []).map((w) => {
    const key = String(w.name || '')
      .toLowerCase()
      .replace(/\./g, '')
    const live = byName.get(key)
    if (!live) return { ...w, board: 'weekly' }

    const std = num(live.projected_ppg_std ?? live.projectedPpgStd, null)
    const half = num(live.projected_ppg_half_ppr ?? live.projectedPpgHalfPpr, null)
    const ppr = num(live.projected_ppg_ppr ?? live.projectedPpgPpr, null)
    const dStd = num(live.draft_value_score_std ?? live.draftValueScoreStd, null)
    const dHalf = num(
      live.draft_value_score_half_ppr ?? live.draftValueScoreHalfPpr,
      null,
    )
    const dPpr = num(live.draft_value_score_ppr ?? live.draftValueScorePpr, null)

    return {
      ...w,
      board: 'weekly',
      projected_ppg: half ?? w.projected_ppg,
      projected_ppg_std: std ?? w.projected_ppg_std,
      projected_ppg_half_ppr: half ?? w.projected_ppg_half_ppr,
      projected_ppg_ppr: ppr ?? w.projected_ppg_ppr,
      draft_value_score: dHalf ?? w.draft_value_score,
      draft_value_score_std: dStd ?? w.draft_value_score_std,
      draft_value_score_half_ppr: dHalf ?? w.draft_value_score_half_ppr,
      draft_value_score_ppr: dPpr ?? w.draft_value_score_ppr,
      projectedPpgStd: std ?? w.projectedPpgStd,
      projectedPpgHalfPpr: half ?? w.projectedPpgHalfPpr,
      projectedPpgPpr: ppr ?? w.projectedPpgPpr,
      draftValueScoreStd: dStd ?? w.draftValueScoreStd,
      draftValueScoreHalfPpr: dHalf ?? w.draftValueScoreHalfPpr,
      draftValueScorePpr: dPpr ?? w.draftValueScorePpr,
      injury: live.injury || live.liveStatus || w.injury,
      liveStatus: live.liveStatus || w.liveStatus,
      reliability: live.reliability && live.reliability !== '—' ? live.reliability : w.reliability,
      reliabilityTier:
        live.reliabilityTier && live.reliabilityTier !== '—'
          ? live.reliabilityTier
          : w.reliabilityTier,
      tier: live.tier && live.tier !== '—' ? live.tier : w.tier,
      age: live.age ?? w.age,
      careerSeasons: live.careerSeasons ?? w.careerSeasons,
      adp_signal: live.adp_signal || w.adp_signal,
      adpSignal: live.adpSignal || w.adpSignal,
      consensus_adp: live.consensus_adp ?? w.consensus_adp,
      consensus_adp_overall: live.consensus_adp_overall ?? w.consensus_adp_overall,
      consensus_adp_positional:
        live.consensus_adp_positional ?? w.consensus_adp_positional,
      our_positional_rank: live.our_positional_rank ?? w.our_positional_rank,
      adp_diff_positional: live.adp_diff_positional ?? w.adp_diff_positional,
      adp_diff_overall: live.adp_diff_overall ?? w.adp_diff_overall,
      consensusAdpOverall: live.consensusAdpOverall ?? w.consensusAdpOverall,
      consensusAdpPositional:
        live.consensusAdpPositional ?? w.consensusAdpPositional,
      ourPositionalRank: live.ourPositionalRank ?? w.ourPositionalRank,
      adpDiffPositional: live.adpDiffPositional ?? w.adpDiffPositional,
    }
  })
}
...
/**
 * Weekly rows with live format columns attached (not yet format-sorted).
 * RankingsBoard re-ranks these client-side for Standard / Half PPR / PPR.
 */
export function getWeeklyRankingsRaw() {
  if (!weeklyRankingsExists()) return []
  return mergeWeeklyWithLive(parseWeeklyPlayers(), parseLivePlayers()).map((p) => ({
    ...p,
    board: 'weekly',
  }))
}
 ...
 * Merges live format columns so Standard / Half PPR / PPR can re-rank.
 */
export function getWeeklyRankings(format = 'half_ppr', options = {}) {
  const merged = getWeeklyRankingsRaw()
  const fmt = normalizeScoringFormat(format || 'half_ppr')
  return rankPlayersByFormat(merged, fmt, {
    superflex: Boolean(options.superflex),
    forceValueSort: true,
 ...
 */
export function getBoardRankings(board, format = 'half_ppr', options = {}) {
  const b = normalizeBoard(board)
  if (b === 'weekly') {
    if (!weeklyRankingsExists()) return []
    return getWeeklyRankings(format, options)
  }
  const players = parseLivePlayers()
  const fmt = normalizeScoringFormat(format || 'half_ppr')
