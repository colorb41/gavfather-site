...
/**
 * Highest week number across all ranking CSVs.
 */
export function getLatestWeek() {
  const files = listRankingFiles()
  if (!files.length) return null
  return Math.max(...files.map((f) => f.week))
}
...
/**
 * Human-readable board label. Draft board (week index 3) uses preseason framing.
 */
export function formatWeekLabel(week, year) {
  if (Number(week) === 3) return '2026 Draft Rankings'
...
export function formatWeekShort(week) {
  if (Number(week) === 3) return 'Preseason 2026'
  return `Week ${week}`
}
