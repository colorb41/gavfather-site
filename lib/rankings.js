 ...
 *
 * Prefers per-format columns:
 *   projected_ppg_std / half_ppr / ppr
 *   draft_value_score_std / half_ppr / ppr
 * Falls back to legacy projected_ppg / draft_value_score when missing.
 *
 * Default display fields use Standard (std).
 */
function normalizeLiveRow(row, index) {
  ...
  return {
    ...
    // Per-format scores (snake_case keys used by rankPlayersByFormat)
    projected_ppg_std,
    projected_ppg_half_ppr,
    projected_ppg_ppr,
    ...
    // Active display fields (Standard by default)
    score: projectedPpg,
    projectedPpg,
...
function readLiveCsv() {
  ...
  // Baseline sort: Standard draft value (1-QB), then apply positional ranks
 ...
 * Always prefers live_rankings.csv, then re-ranks for format.
 */
 ...
 * (Position tabs unlock top 5 client-side in RankingsBoard.)
 */
  ...
  return [
    {
      week: 0,
      year,
      // Scoring formats only — Superflex is roster construction (separate toggle)
