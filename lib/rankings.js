...
function normalizeLiveRow(row, index) {
  ...
  const csvOverallRank = num(row.overall_rank, index + 1) ?? index + 1

  return {
    rank: csvOverallRank,
    overall_rank: csvOverallRank,
    // Preserved CSV overall_rank (rankPlayersByFormat may overwrite rank/overall_rank)
    csvOverallRank,
    name: str(row.player_name),
    team: str(row.recent_team || row.team).toUpperCase(),
    position: str(row.position).toUpperCase(),
    // Fade / ADP signal columns when present in the CSV
 ...
 * Sort by CSV overall_rank (standard draft board order) — not re-ranked draft_value.
 */
export function getTopPlays(_week, n = 5) {
  return getLiveRankings()
    ...
    .map((p, i) => ({
      ...p,
      // Display CSV board rank on homepage cards
...
export function getPreviewPlayers() {
  return getLiveRankings()
    ...
    .sort(
      (a, b) =>
        (a.csvOverallRank ?? a.rank) - (b.csvOverallRank ?? b.rank),
    )
    .slice(0, 20)
}
...
export function getTopFades(_week, n = 5) {
  ...
  if (hasOutlookFade || hasAdpSignal) {
    ...
    // Our rank significantly worse than consensus ADP → overvalued / fade
    fades = relevant.filter((p) => {
